...
struct BlobData{
	...
	int height;
	int width;
	int capacity_count;		//The number of elements of the reserved space, the number of bytes * sizeof(float)
};
...
//Connect the body area according to the results obtained
...
//bottom_blob Is input，top Is the output
void nms(BlobData* bottom_blob, BlobData* top_blob, float threshold){
	//maxPeaks Is the maximum number, +1 is for the first number
	//The algorithm is that each point is considered to be a peak if it is greater than the threshold and is greater than the upper, lower, left and right values

	//The algorithm is simple，featuremap Any point that is up, down, left, and right, and up and down, is less than itself, and is considered to be the desired point
	//Then use the point area to select the 7*7 area and calculate the most suitable sub-pixel coordinates according to the score value and x, y
...
vector<Point> renderKeypointsCpu(Mat& frame, const vector<float>& keypoints, vector<int> keyshape, const std::vector<unsigned int>& pairs,
...
vector<Point> renderPoseKeypointsCpu(Mat& frame, const vector<float>& poseKeypoints, vector<int> keyshape,
	const float renderThreshold, float scale, const bool blendOriginalFrame = true)
{
  ...
  vector<Point> finalKeypoints;
	...
	// Render keypoints
	finalKeypoints = renderKeypointsCpu(frame, poseKeypoints, keyshape, pairs, POSE_COCO_COLORS_RENDER, thicknessCircleRatio,
...
int main(int argc, char** argv){
	...
	//Get a frame of image, according to the agreed size, this method is to ensure that the aspect ratio of the image is unchanged
	im = getImage(raw_image, baseSize, &scale);
	
	//This step converts plus or minus the mean, manual operation
	...
	for (int i = 0; i < net_output->channels; ++i){
		...
		//Featuremap's resize interpolation method is very relevant
	...
	//Get the local maxima of each feature map
	nms(input, nms_out, 0.05);

	//After obtaining the local maximum, the components are connected according to PAFs and points.
